...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateInput(InputSettings)
			...
			local width = InputSettings.Width or 120
			local minWidth = InputSettings.MinWidth or width

			local function UpdateInputSize()
				Input.InputFrame.Size = UDim2.new(0, math.max(width, minWidth), 0, 30)
			end
			...
			Input.InputFrame.InputBox.Size = UDim2.new(1, -10, 1, 0)
			Input.InputFrame.InputBox.TextXAlignment = Enum.TextXAlignment.Left
			Input.InputFrame.InputBox.ClearTextOnFocus = false
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			Dropdown.Selected.TextScaled = false
			Dropdown.Selected.AutomaticSize = Enum.AutomaticSize.None
			...
			Dropdown.Selected.TextWrapped = false
			Dropdown.Selected.AutomaticSize = Enum.AutomaticSize.None

			local SearchBox
			if DropdownSettings.Searchable then
				...
				SearchBox.ZIndex = 50
				SearchBox.Parent = Dropdown.List
			end

			local function FilterDropdown(text)
				...
				for _, child in ipairs(Dropdown.List:GetChildren()) do
					if child.ClassName == "Frame" and child.Name ~= "Placeholder" then
						child.Visible =
							query == ""
							or string.find(string.lower(child.Name), query, 1, true)
					end
				end

				Dropdown.List.CanvasSize = UDim2.new(
					0, 0,
					0, Dropdown.List.UIListLayout.AbsoluteContentSize.Y + 8
				)
			end

			if SearchBox then
				SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
					FilterDropdown(SearchBox.Text)
				end)
			end
			...
			local function SetDropdownOptions()
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Title.TextScaled = false
					DropdownOption.Title.TextSize = 14
